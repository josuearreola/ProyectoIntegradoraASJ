...
$sqlCompra = mysqli_query($conexion, "SELECT venta.id_vta, fec_vta, cant_pago, tip_pago FROM venta INNER JOIN pago ON venta.id_vta = pago.id_vta WHERE venta.id_vta = $idVta");
...
$insertFact = mysqli_query($conexion, "INSERT INTO factura(fec_fact,id_vta) values ('$fechaFact','$idVta')");
$idFact = mysqli_insert_id($conexion);
...
$pdf->SetXY(142, 24);
$pdf->Cell(0, 10, 'No. Factura: ' . $idFact, 0, 1);
$pdf->SetXY(142, 30);
...
if ($totalSession != $totalDescSession) {
    ...
    $pdf->Cell(35, 10, MONEDA . ' ' . number_format($totalDescSession, 2, '.', ','), 1, 0, 'R');
